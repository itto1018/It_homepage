...
import { getSkillLevelDescription, Skill, SkillLevel } from "@/types/serviceSkill";
...
export const ServicesEditor: React.FC<Props> = ({ initialSkills }) => {
  ...
  const handleSkillChange = (
    ...
    setSkills(skills.map(skill => {
      ...
      return {
        ...skill,
        [field]: field === 'level' ? Number(value) as SkillLevel : String(value)
      };
    }));
  };
  ...
  return (
    <div className="space-y-8">
      {services.map((service) => {
        ...
        const serviceSkills = skills.filter(skill => skill.serviceId === service.id);
        
        return (
          <div key={service.id} className="rounded-lg border border-[#00a497]/10 bg-white p-6 shadow-sm">
                ...
                >
                  <div className="flex items-center gap-4">
                    <input
                      type="text"
                      value={skill.name}
                      onChange={(e) => handleSkillChange(service.id, skill.id, "name", e.target.value)}
                    ...
                    <select
                      value={skill.level}
                      onChange={(e) => handleSkillChange(service.id, skill.id, "level", e.target.value)}
                      className="w-24 rounded-lg border border-gray-200 px-4 py-2"
                      disabled={isLoading}
                    >
                      {[1, 2, 3, 4, 5].map((level) => (
                        <option key={`${service.id}-${skill.id}-${level}`} value={level}>
                          Lv{level}
                        </option>
                      ))}
                    </select>
